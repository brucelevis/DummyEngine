...
void Physics::Read(const JsObject &js_in, Physics &ph) {
    using Phy::real;
    ...
    if (js_in.Has("vel")) {
        const JsArray &js_vel = js_in.at("vel").as_arr();
    ...
    {
        const JsObject &js_shape = js_in.at("shape").as_obj();
        const JsString &js_shape_type = js_shape.at("type").as_str();
...
void Physics::Write(const Physics &ph, JsObject &js_out) {}